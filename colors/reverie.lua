...
-- For more advanced configuration, use: require('reverie').setup()

-- Check if the user wants to use the modular system
local ok, reverie_colorscheme = pcall(require, "reverie")
if ok then
	-- Use the modular system with default configuration
	reverie_colorscheme.setup()
	reverie_colorscheme.load()
	return
end
...
-- Color palette from cerulean theme
local colors = {
	-- Base colors
	...
	bg_visual = palette.bright_black,
	...
	string_template = palette.yellow,
	string_escape = palette.yellow,
	string_regex = palette.yellow,
	...
	punctuation = palette.bright_black,

	-- Special colors
	cursor = palette.foreground,
	line_number = palette.bright_black,
	line_number_active = palette.foreground,
	...
	git_delete = palette.red,
	git_ignore = palette.bright_black,
